...
function createReaderHtml({
  ...
  if (work.readerHtmlIsCompleteDocument && work.readerHtml) {
    return createCbetaApiReaderHtml({
      html: work.readerHtml,
      theme,
      readerItems,
      activeItemId,
      nextWorkTitle,
...
function createCbetaApiReaderHtml({
  html,
  theme,
  readerItems,
  activeItemId,
  ...
  const activeAnchorId = JSON.stringify(
    readerItems.find((item) => item.id === activeItemId)?.block.anchorId ?? "",
  );
  const apiEndPanel = `<section id="reader-end-panel" class="reader-end-panel"><h2>已到本部末尾</h2><p>${
    nextWorkTitle ? `下一部：${escapeHtml(nextWorkTitle)}` : "已到经藏末尾"
  }</p>${nextWorkTitle ? `<button id="next-work">下一部</button>` : ""}</section>`;
...
(() => {
  const readerMetadata = ${readerMetadata};
  const activeAnchorId = ${activeAnchorId};
  const metadataByAnchor = new Map(readerMetadata.map((item) => [item.anchorId, item]));
  const blockAnchorIds = new Map();
  ...
  const readerBlocks = () => Array.from(document.querySelectorAll("p, .lg-row")).filter((node) => {
    return (node.textContent || "").trim().length >= 2;
  });
  const buildAnchorMap = () => {
    blockAnchorIds.clear();
    let currentAnchorId = "";
    const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_ELEMENT);
    let node = walker.currentNode;
    while (node) {
      if (node.classList?.contains("lb") && node.id) {
        currentAnchorId = node.id;
      }
      if ((node.matches?.("p, .lg-row")) && (node.textContent || "").trim().length >= 2) {
        blockAnchorIds.set(node, currentAnchorId);
      }
      node = walker.nextNode();
    }
  };
  const selectBlock = (block) => {
    document.querySelectorAll(".reader-selected").forEach((node) => {
      node.classList.remove("reader-selected");
    });
    block.classList.add("reader-selected");
  };
  const blockForAnchor = (anchorId) => {
    if (!anchorId) {
      return null;
    }
    for (const [block, candidateAnchorId] of blockAnchorIds.entries()) {
  ...
  const messageForBlock = (block, type) => {
    const anchorId = blockAnchorIds.get(block) || "";
    const item = metadataByAnchor.get(anchorId);
    if (!item) {
      return null;
    }
    const rect = block.getBoundingClientRect();
    ...
    return {
      type,
      itemId: item.id,
      charOffset: Math.floor(item.charStart + (item.charEnd - item.charStart) * ratio),
      scrollFraction: scrollFraction()
    };
  };
  const initializeReader = () => {
    buildAnchorMap();
    const hash = decodeURIComponent(window.location.hash.replace(/^#/, ""));
    const targetAnchor = hash ? document.getElementById(hash) : null;
    const activeBlock = blockForAnchor(hash || activeAnchorId);
    if (activeBlock) {
      selectBlock(activeBlock);
    }
    if (targetAnchor) {
      window.setTimeout(() => {
        targetAnchor.scrollIntoView({ block: "start", inline: "nearest" });
      }, 0);
    }
  };
  const clearSearchHit = () => {
  ...
  const goToSearchMatch = (index) => {
    ...
    searchMatchIndex = (index + searchMatches.length) % searchMatches.length;
    const block = searchMatches[searchMatchIndex];
    selectBlock(block);
    block.classList.add("search-hit");
    block.scrollIntoView({ block: "start", inline: "nearest" });
    window.setTimeout(() => {
      const message = messageForBlock(block, "select");
      if (message) {
        post(message);
      }
      reportSearch(true);
  ...
  window.__sutraSearch = (query) => {
    ...
    for (const block of readerBlocks()) {
      if ((block.textContent || "").includes(term)) {
        searchMatches.push(block);
      }
    }
    if (!searchMatches.length) {
      reportSearch(false);
  ...
  document.addEventListener("click", (event) => {
    ...
    const block = event.target.closest("p, .lg-row");
    if (!block) {
      return;
    }
    selectBlock(block);
  ...
  window.addEventListener("scroll", () => {
    ...
    window.requestAnimationFrame(() => {
      ticking = false;
      const block = readerBlocks().find((candidate) => {
        const rect = candidate.getBoundingClientRect();
        return rect.bottom >= 28;
      });
      if (block) {
        const message = messageForBlock(block, "position");
        if (message) {
          post(message);
        }
      }
  ...
  return html
    .replace(/<\/head>/i, `${apiStyle}</head>`)
    .replace(/<\/body>/i, `${apiEndPanel}${apiScript}</body>`);
}
